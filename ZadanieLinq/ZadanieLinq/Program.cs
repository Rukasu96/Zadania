...
var sortedNames = users.Select(x => x.Name).ToList();
sortedNames.Sort();
...
var marksCount = users.Where(x => x.Marks != null && x.Marks.Count() > 0).ToList();
Console.WriteLine(marksCount.Count());
...
//Najlepsza ocena
var bestMarks = marksCount.Select(x => x.Marks.Max()).ToList();
...
//Najgorsza ocena
var worstMarks = marksCount.Select(x => x.Marks.Min()).ToList();
...
Console.WriteLine();
var groupsByMonth = users.GroupBy(x => x.CreatedAt.Value.Month).ToList();
